...
def create_gradient_background(width, height, color1=OBSIDIAN, color2=ONYX, vertical=True):
...
def create_logo_png():
    ...
    draw.line([(size-45, size-45), (size//2, size//2)], fill=(*CHAMPAGNE, 50), width=1)
...
def create_icon_set():
    ...
    for icon_name in icons:
        ...
        if icon_name == 'diamond':
            ...
            # Shield shape
            points = [(cx-20, cy-24), (cx+20, cy-24), (cx+20, cy), (cx, cy+24), (cx-20, cy)]
            draw.polygon(points, outline=CHAMPAGNE, width=2)
        elif icon_name == 'crown':
            # Crown/castle top
            points = [(cx-20, cy+20), (cx-20, cy-10), (cx-10, cy-10), (cx-10, cy-20), 
                     (cx, cy-5), (cx+10, cy-20), (cx+10, cy-10), (cx+20, cy-10), (cx+20, cy+20)]
            draw.polygon(points, outline=CHAMPAGNE, width=2)
        elif icon_name == 'star':
            # 5-pointed star
            points = []
...
def create_stat_card_preview():
    ...
    # Top-right  
    draw.line([(width-corner_size, 0), (width, 0), (width, corner_size)], fill=CHAMPAGNE, width=1)
    # Bottom-left
    draw.line([(0, height-corner_size), (0, height), (corner_size, height)], fill=CHAMPAGNE, width=1)
    # Bottom-right
    draw.line([(width-corner_size, height), (width, height), (width, height-corner_size)], fill=CHAMPAGNE, width=1)
