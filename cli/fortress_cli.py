...
def ensure_config():
    ...
    if not CONFIG_FILE.exists():
        config = {
            "user": {"id": None, "name": None, "squad": None},
            "api": {"endpoint": "http://localhost:3000", "key": None},
            "theme": "light",
        }
...
def cmd_status(args):
    ...
    print(
        f"{Colors.BOLD}⚔ FATED FORTRESS CLI{Colors.RESET} | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print(DIVIDER)
    ...
    print("  fortress quests list    - View all quests")
    print("  fortress squads list    - View all squads")
    print("  fortress leaderboard    - View top builders")
    print("  fortress quests create  - Create new quest")
    print()
...
def cmd_quests(args):
    ...
    if args.quest_action == "list":
        ...
        # Table header
        print(
            f"{Colors.BORDER}ID{Colors.RESET:>6} │ {Colors.BORDER}Domain{Colors.RESET:>10} │ {Colors.BORDER}Title{Colors.RESET:>30} │ {Colors.BOND}Bond{Colors.RESET:>6} │ {Colors.STATUS}Status{Colors.RESET}"
        )
        print(DIVIDER)
        ...
        for q in quests:
            ...
            domain_color = (
                ...
                if q["domain"] == "BACKEND"
                    ...
                    Colors.ACCENT_WARM
                    if q["domain"] == "FRONTEND"
                    else Colors.TEXT_SECONDARY
                )
            )

            print(
                f"{q['id']:>6} │ {domain_color}{q['domain']:>10}{Colors.RESET} │ {q['title'][:28]:>30} │ {q['bond']:>6} │ {status_color}{q['status']:>12}{Colors.RESET}"
            )
        ...
        if not args.title:
            ...
            print(
                'Usage: fortress quests create --title "Your quest title" --domain BACKEND'
            )
            sys.exit(1)
...
def cmd_squads(args):
    ...
    for s in squads:
        ...
        print(
            f"              Members: {s['members']} │ Score: {s['score']} │ {status_color}{status_icon} {s['status']}{Colors.RESET}"
        )
...
def cmd_leaderboard(args):
    ...
    for leader in leaders:
        rank_emoji = (
            "🥇"
            if leader["rank"] == 1
            else "🥈" if leader["rank"] == 2 else "🥉" if leader["rank"] == 3 else "  "
        )
        print(
            f"  {rank_emoji} #{leader['rank']} {Colors.ACCENT_GOLD}{leader['name']:>12}{Colors.RESET} [{leader['squad']:>7}] │ REP: {leader['rep']:>5} │ Quests: {leader['quests']}"
        )
...
def cmd_submit(args):
    ...
    if not args.quest_id or not args.content:
        print(
            f"{Colors.ERROR}Error: Both --quest-id and --content required{Colors.RESET}"
        )
        sys.exit(1)
...
Examples:
  ...
  fortress login --name "YourName"
    ...
    quest_sub.add_parser("list", help="List all quests")
    ...
    quest_create.add_argument(
        "--domain", choices=["BACKEND", "FRONTEND", "DEVOPS"], default="BACKEND"
    )
