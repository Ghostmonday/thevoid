 ...
 */
export function extractTicketId(prBody) {
    if (!prBody)
        return null;
    const patterns = [
        /ticket-([a-f0-9\-]{36})/i,
 ...
 */
export function mapPRActionToSystemEvent(action, pr) {
    switch (action) {
        case 'opened':
...
export function createWebhookEvent(source, action, userId, payload) {
    return {
        source,
        action,
        userId,
